...
export default function ForgotVerifyPage() {
  ...
  return <AuthShell progress={{ step: 2, label: "Tasdiqlash kodi" }} eyebrow="XAVFSIZ TASDIQLASH" title="Kodni kiriting" subtitle="Kontaktga yuborilgan 6 xonali kodni kiriting."><form className="auth-form-upgraded" onSubmit={submit}><label className="auth-field"><span>Tasdiqlash kodi</span><OtpInput value={code} onChange={setCode} error={error}/></label><p className="auth-help">Kod cheklangan vaqt davomida amal qiladi.</p>{error && <InlineAlert id="otp-error">{error}</InlineAlert>}<button className="button button-primary auth-primary" disabled={busy || code.length !== 6}>{busy ? "Tekshirilmoqda…" : "Kodni tasdiqlash"}</button><div className="auth-dual-links"><Link href="/forgot-password">← Ma’lumotni o‘zgartirish</Link><button type="button" disabled={seconds > 0 || busy} onClick={sendAgain}>{seconds > 0 ? `Qayta yuborish 00:${String(seconds).padStart(2,"0")}` : "Kodni qayta yuborish"}</button></div></form></AuthShell>;
}
