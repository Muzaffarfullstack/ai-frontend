...
export default function ProfilePage() {
  ...
  const joined = user?.created_at ? new Intl.DateTimeFormat(locale === "uz" ? "uz-UZ" : locale, { day: "numeric", month: "long", year: "numeric" }).format(new Date(user.created_at)) : "â€”";

  return <div className="profile-page"><header className="student-page-heading"><span className="lime-label">HISOB SOZLAMALARI</span><h1>{t("profile.title")}</h1><p>{t("profile.subtitle")}</p></header>{error && <div className="form-error page-alert">{error}</div>}{saved && <div className="success-message page-alert">Maâ€™lumotlar saqlandi.</div>}
    <section className="panel profile-summary"><div className="profile-identity"><div className="profile-avatar">{user?.first_name.slice(0, 1)}</div><div><h2>{user?.first_name} {user?.last_name}</h2><p>{user?.role === "admin" ? "Administrator" : user?.role === "mentor" ? "Mentor" : "Oâ€˜quvchi"}</p>{user?.email_verified_at && <span className="verified-badge"><AppIcon name="check" size={15}/>Email tasdiqlangan</span>}</div></div><div className="profile-completion"><span>Profil toâ€˜liqligi</span><strong>{completion}%</strong><div className="progress-track"><i style={{ width: `${completion}%` }}/></div><small>{completion < 100 ? "Yetishmayotgan maâ€™lumotlarni qoâ€˜shing" : "Profilingiz toâ€˜liq"}</small></div><div className="profile-summary-item"><AppIcon name="calendar"/><span>Qoâ€˜shilgan sana<strong>{joined}</strong></span></div><div className="profile-summary-item"><AppIcon name="shield"/><span>Hisob holati<strong>{user?.is_active ? "Faol" : "Faol emas"}</strong></span></div></section>
    <section className="profile-content-grid"><form className="panel personal-form" key={user?.updated_at ?? user?.id} onSubmit={submit}><h2>Shaxsiy maâ€™lumotlar</h2><div className="form-grid"><label className="field"><span>{t("profile.firstName")}</span><input name="first_name" required defaultValue={user?.first_name}/></label><label className="field"><span>{t("profile.lastName")}</span><input name="last_name" defaultValue={user?.last_name ?? ""} placeholder="Familiyangiz"/></label><label className="field"><span>{t("profile.email")}</span><input disabled value={user?.email ?? "Email kiritilmagan"}/></label><label className="field"><span>{t("profile.phone")}</span><input disabled value={user?.phone_number ?? "Telefon kiritilmagan"}/></label><label className="field field-wide"><span>{t("profile.language")}</span><select value={locale} onChange={(event) => setLocale(event.target.value as Locale)}>{(Object.keys(localeNames) as Locale[]).map((item) => <option key={item} value={item}>{localeNames[item]}</option>)}</select></label></div><footer><small>Oâ€˜zgarishlar faqat â€œSaqlashâ€ tugmasidan keyin qoâ€˜llanadi.</small><button className="button button-primary">Oâ€˜zgarishlarni saqlash</button></footer></form>
      <div className="profile-right-stack"><article className="panel contact-card"><h2>Bogâ€˜lanish maâ€™lumotlari</h2><div className="contact-row"><span><AppIcon name="mail"/></span><div><b>Email</b><small>{user?.email ?? "Kiritilmagan"}</small></div><i className={user?.email_verified_at ? "verified" : "unset"}>{user?.email_verified_at ? "Tasdiqlangan" : "Sozlanmagan"}</i><button onClick={() => openContact("email")}>Yangilash</button></div><div className="contact-row"><span><AppIcon name="phone"/></span><div><b>Telefon</b><small>{user?.phone_number ?? "Kiritilmagan"}</small></div><i className={user?.phone_verified_at ? "verified" : "unset"}>{user?.phone_verified_at ? "Tasdiqlangan" : "Sozlanmagan"}</i><button onClick={() => openContact("phone")}>Telefon qoâ€˜shish</button></div>
        ...
        <p>Aloqa maâ€™lumotini almashtirish tasdiqlash kodi orqali bajariladi.</p></article>
        <article className="panel security-panel"><h2>Xavfsizlik</h2><button onClick={() => setPasswordOpen((open) => !open)}><span><AppIcon name="shield"/></span><div><b>Parolni yangilash</b></div><i>â€º</i></button>{passwordOpen && <form className="password-form" onSubmit={changePassword}><input name="current_password" type="password" required placeholder="Joriy parol"/><input name="new_password" type="password" minLength={8} required placeholder="Yangi parol"/><button className="button button-primary">Yangilash</button></form>}<div className="security-row"><span><AppIcon name="clock"/></span><div><b>Faol qurilma</b><small>Hozirgi sessiya</small></div></div><button className="logout-all" onClick={() => void logout()}>Barcha sessiyalardan chiqish</button></article>
      </div>
    </section>
  </div>;
}
